...
**说明书双语，并且能在界面里直接读、即时切换语言。**

1.0.0 是首发；这一版把「说明」这件事补齐 —— 之前 README 是一份双语合订本，而且想读它就得
离开界面去找文件，所以 1.0.0 的 exe 里也还没有这个入口。

### 新增 / Added

1. **两份 README**：`README.md`（英文，GitHub 默认展示）与 `README.zh-CN.md`（中文），各自顶部有
   语言互链。内容已对齐 —— 英文侧补上了原来只有中文的「界面语言与翻译管线」一节。
2. **界面里的「关于」面板**：顶部按钮打开一个覆盖层，用界面自己的 markdown 渲染器渲染整份文档；
   **中/英切换是换一份已取回的文本**（两份都缓存在内存里），不跳页、不刷新、不重复请求。
   语言默认跟随界面语言，之后各自独立；Esc 或点空白处关闭。
3. **接口 `GET /api/readme?lang=en|zh`**：两个固定文件名（不接受任意路径），未知语言回落英文，
   并返回 `available` 让界面知道该显示几个语言选项。
4. 打包侧：`app/README.md` 与 `app/README.zh-CN.md` 都进包，且被 `verify-release` 列为**必需文件**。

### 本次验证 / Verification evidence

| 检查 | 命令 | 结果 |
| --- | --- | --- |
| 快速自检 | `npm run verify:fast` | 全过（26 步） |
| README 接口 | `npm run traverse` | **85/85**（新增 5 条：两种语言都返回且内容不同、`available` 正确、未知语言回落、正文长度合理） |
| 界面面板 | `npm run traverse:ui` | **210/210**（新增 8 条：按钮打开、正文渲染成真结构、两个语言选项都在、切换后文本确实变了、**没有跳页**、没有整页刷新、Esc 关闭） |
| i18n | `npm run i18n:coverage` / `i18n:proofread` | 25 个地区 **638/638 = 100%**；校对硬失败 **0** |
| 发行链 | `npm run release` | 构建 ✓ · 校对 clean · 两个遍历全过 |

### 尚未验证 / Not yet verified

与 1.0.0 相同：用真实账号发弹幕、用真实 Key 做视觉打标都需要使用者自己的凭据，自检一律用本地假服务。

---

## v1.0.0 — 对外首发 / first public release
...
```
VtuberMonitorLink-1.0.0-win-x64.zip     ~40 MB，解压后双击
  VtuberMonitorLink.exe                 单文件，自带运行时，不需要装 Node、不需要管理员权限
```

> 版本号说明：开发期的编号是 1.0 → 1.7.x（记录在下面的「历史」里），
> **对外首发重新从 1.0.0 起算** —— 下载页看到的 1.0.0 就是这个工程的全部能力第一次公开。
> 命名规则也一并固定下来：**面向使用者的文件用全名 `VtuberMonitorLink`，内部标识用短名 `VML`**
> （`npm run brand` 会检查这条规则，见 `tools/vml-brand.mjs`）。

### 这一版有什么

| 能力 | 要点 |
| --- | --- |
| 三十个内置来源 | Reddit / Fandom / 萌娘百科 / Twitch / X / YouTube / B 站动态 / 各家官方 NEWS / 通贩平台… 逐条勾选，也可可视化新增 |
| 监视对象 | 任意网页 / MediaWiki 条目 / 最近更改 / 监视列表 / B 站动态；给「变了什么、变了多少」，不是只说「变了」 |
| 按人关注 | 本地字符串匹配归属到人，命中带证据；可从 VDB 花名册（多平台、社团维度）一键导入 |
| 事件合并 | IDF 加权相似度 + 并集单链接 + 时间窗，标出「几个来源确认」 |
| 箱视角 / 静默 / 休眠 | 社团热力图与同刻出现；按**各人自己的节奏**判停更；≥6 个月无动静的进日报末尾 |
| 观测模式 | 抽样 + 抖动 + 只对日志归属在对方的站点走 Tor，降低「有人把整箱扫了一遍」的痕迹 |
| 推送与静默时段 | 12 种渠道；静默期内的通知入队补发而不是丢弃 |
| 报告与归档 | SQLite 增量归档、内联 SVG 图表、纪念日倒计时、一键分享成单文件、导出 Word/Excel |
| 25 个地区 | 含 RTL、日期/数字/一周起始日按地区、数词词形按 `Intl.PluralRules` |
| 用量看板 | 按次按模型记账，可选每日预算，超限可拦；拿不到用量的调用单独计数 |
| 桌面版形态 | 单文件 exe + 本地网页界面（`127.0.0.1:43110`），不依赖任何在线服务；LLM 可选 |
| 双语说明 | `README.md`（英文）与 `README.zh-CN.md`（中文）；**界面里右上角「关于」直接读**，并且可以即时切换语言（覆盖层，不跳页、不刷新） |

### 追加：双语 README + 界面内「关于」面板

说明书原来只有一份双语合订本，而且想读它就得离开界面去找文件。现在：

- **两份文档**：`README.md`（英文，GitHub 默认展示）与 `README.zh-CN.md`（中文），各自顶部有语言互链。
  内容对齐（英文侧补上了原来只有中文的「界面语言与翻译管线」一节）。
- **界面内阅读**：顶部「关于」按钮打开一个覆盖层，用界面自己的 markdown 渲染器渲染整份文档；
  中/英切换是**换一份已取回的文本**（两份都缓存在 state 里），不跳页、不刷新、不重新请求。
  语言默认跟随界面语言，之后各自独立 —— 用日语界面的人也可能想读中文那份。
- **接口**：`GET /api/readme?lang=en|zh`（两个固定文件名，不接受任意路径；未知语言回落英文；
  返回 `available` 让界面知道该显示几个选项）。
- 打包侧：`app/README.md` 与 `app/README.zh-CN.md` 都进包，且被 `verify-release` 列为必需文件 ——
  少一个的话，面板的语言切换就没有东西可切，而这种缺失只会在使用者点开时才发现。

| 检查 | 结果 |
| --- | --- |
| README 接口 | traverse **85/85**（新增 5 条：两种语言都返回、内容确实不同、`available` 正确、未知语言回落、正文长度合理） |
| 界面内面板 | traverse:ui **210/210**（新增 8 条：按钮打开、正文渲染成真结构、两个语言选项都在、切换后文本确实变了、**没有跳页**、没有整页刷新、Esc 关闭） |

### 发布前检查（1.0.0 实测）

| 检查 | 命令 | 结果 |
| --- | --- | --- |
| 快速自检（26 步） | `npm run verify:fast` | 全过 |
| 命名一致性 | `npm run brand` | 内部标识 **9/9** VML、对外件 **7/7** 全名 |
| 英文覆盖率 | `npm run english` | 工程层 **100%**（5923 项：3475 注释块 + 2448 输出串，0 处中文）；界面词条 **100%**（25 个地区 × 633 键） |
| 无痕化 | `npm run sanitize-check` + `npm run release` | 无硬编码路径 / 凭据 / 私人名字；发行树重扫通过 |
| 发行链 | `npm run release` | 构建 ✓ · 校对 clean · 端点遍历 **80/80** · UI 遍历 **202/202** |
| 发行包 | `tools/make-zip.mjs` | 包内无 `config.json` / 报告 / 日志 / 花名册缓存 |

### 装起来要注意的

- **不需要装 Node**，也不需要管理员权限；解压到任意目录双击即可。
- 需要登录的来源（B 站带配图动态、X 正文、萌百监视列表）**由你自己的浏览器登录**，程序只读地
  复制 cookie 库来取登录态，不打包、不上传任何账号。
- 视觉打标（把配图发给外部模型）**默认关闭**；LLM 分析与弹幕发送都需要你显式开启并确认。
- 打包时 postject 会打印 `warning: The signature seems corrupted!` —— 它对 node 基线签名的既有提示，
  `--doctor` 与两个遍历都通过，属已知的无害提示。

---

## 历史 / history（开发期编号，不对外发布）
