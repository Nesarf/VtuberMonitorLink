...
| # | 现象 | 根因 | 状态 |
| --- | --- | --- | --- |
| 22 | 推送自检里两条诡异失败：① 「首次去重判断就返回重复」；② 「静默期内不该投递却投递了」，且队列莫名是空的 | ① 我把测试目录写成 `cfg.logsDir`，而 `resolveDir` 读的是 **`cfg.paths.logsDir`** —— 测试于是在**静默地读写仓库里真实的 logs 目录**，上一次运行留下的去重记录与队列污染了这一次；② 我把 `"23"`（只有小时）当 `"HH:MM"` 传给 `toMinutes`，被判非法 → 走 fail-open 直接推送 | **已修**（目录放进 `cfg.paths`；断言里先验证 `inQuietHours` 判定为 `true` 再测投递。并把 **fail-open 写成断言**：配置写坏时照常推送，因为「配错导致所有通知消失」比「偶尔半夜响一下」严重得多） |
