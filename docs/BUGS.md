...
| # | 现象 | 根因 | 状态 |
| --- | --- | --- | --- |
| 25 | 发布流水线第一次运行**拒绝发布**，报了 13 处 | 多数是「用发行版校验器去扫源码树」造成的误判（源码树当然没有 exe / app/server/node_modules）；另有 2 处是**真问题**：① `package.json` 里有个 em dash（非 ASCII，而它是必须 ASCII 的关键文件）；② 我自己写的 `tools/make-release.mjs` 里**写死了 `E:\VML-r-1.0`** —— 无痕化要抓的就是这种东西 | **已修**（给校验器加正经的 `--scan-only` 模式，而不是在调用方过滤它的输出 —— 过滤校验器的输出等于把它嘴捂住；`package.json` 改纯 ASCII；输出目录改用 `VML_RELEASE_OUT` 或工程同级推导，源码里不留本机路径） |
