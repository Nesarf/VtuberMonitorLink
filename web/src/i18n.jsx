...
export const STRINGS = {
  zh: {
    ...
    shareTitle: '一键分享',
    shareHint:
      '把情报打成一个**单文件**发给朋友：自带样式、没有任何外部引用，对方双击就能看（离线也行）。不需要登录的方式永远可用；需要登录的方法会如实告诉你缺什么，做不到的会直接标成「不支持」而不是给你一个点了没反应的按钮。发到站点的发布页属于对外发声 —— 必须你确认，而本版本没有可自动发布的站点。',
    shareScope: '分享范围',
    ...
    // The three steps of a posting site, each on its own: which account it would use, whether what the site
    // needs is satisfied (measured on demand), and posting itself.
    shareStageAccount: '登录账号',
    shareStageVerify: '验证',
    shareStageSend: '发送',
    shareConfig: '配置',
    shareImages: '图片',
    shareImagesCount: '每个文件最多几张',
    shareAccountPick: '用哪个账号',
    shareSiteAdd: '添加站点',
    shareSiteId: '站点 id',
    shareSiteName: '名称',
    shareSiteLoginKind: '登录方式',
    shareSitePh: '例如 my-site（字母、数字、- 和 _）',
    shareSiteNeeds: '检测到需要',
    shareSiteSave: '添加',
    shareSiteCancel: '取消',
    shareSiteRemove: '删除站点',
    shareSiteLink: '发布页',
    shareManual: '手动发送',
    shareManualHint: '本站没法自动发：正文和配图准备好，复制或打开站点的发布页由你自己发。',
    shareManualCopy: '复制正文',
    shareManualDownload: '下载文件',
    shareManualOpen: '打开发布页',
    shareManualFull: '正文超出本站上限，按钮里不带正文（用复制或下载）',
    shareTextLimit: '本站上限 {n} 字符',
    shareAudit: '分享记录',
    ...
    checkingLogin: '检查中…',
    loginOk: '读到登录 cookie',
    loginNoSession: '读到 cookie，但没有会话 cookie（可能没登录）',
    ...
    cookieCount: '{n} 个',
    cookieCountWithSession: '{n} 个（含会话 cookie）',
    ...
    proxyTitle: '网络代理',
    proxyHint:
      'Node 的 fetch 默认不读系统代理。若你的网络需要代理才能出网（例如国内网络），请在此启用——抓取与浏览器渲染会同时生效。注意：部分站点走代理反而会被拦，可在来源/监视对象上单独设为「直连」。',
    proxyEnabled: '启用代理',
    proxyUrl: '代理地址',
    ...
    obsRotationHint:
      '「箱自托管站点」是指官方站点这类日志留在对方服务器上的入口。Reddit / Fandom 这些平台源不走 Tor：箱看不到那些日志，而实测经 Tor 慢约 8 倍、部分接口还会被限流。换出口用的是 Tor 的 SOCKS 用户名隔离，实测不同用户名会落到不同的出口 IP。',
    obsSampling: '本轮为取样',
    ...
    login_required: '必需',
    customSources: '自定义来源',
    customSourcesHint: '内置来源覆盖不到的站点，可以自己加：给个 RSS、一个 MediaWiki API，或者一个需要浏览器渲染的页面。',
    ...
    watchTitle: '监视对象',
    watchHint:
      '不再只靠站点的「最近更改」流：可以盯住一个具体条目、一个具体页面、一个任意地址。第一次检查只建立基线，之后每次都会给出「改了什么」。',
    watchEnabled: '启用监视',
    checkAll: '全部检查一次',
    showRules: '告警规则',
    hideRules: '收起规则',
    ...
    // intel
    intelTitle: '情报卡片流',
    intelHint: '最近一次运行抓到的条目。命中告警关键词的会打上标记；正文里的 [表情] 会单独标出来，配图直接内联显示。',
    ...
    llmVisionHint: '这是视觉模型 —— 配好之后，分析层可以真的看条目里的配图。',
  ...
  en: {
    ...
    shareTitle: 'Share',
    shareHint:
      'Turn the intel into a **single file** to send someone: styled, with no external references, so it opens by double-click even offline. Ways that need no login always work; ways that need one tell you exactly what is missing, and unsupported methods are labelled rather than offered as a dead button. Posting to a site is speaking in public — it requires your confirmation, and no site in this build can post by itself.',
    shareScope: 'Scope',
    ...
    shareStageVerify: 'Verify',
    shareStageSend: 'Send',
    shareConfig: 'Configure',
    shareImages: 'Images',
    shareImagesCount: 'At most this many per file',
    shareAccountPick: 'Account to use',
    shareSiteAdd: 'Add a site',
    shareSiteId: 'Site id',
    shareSiteName: 'Name',
    shareSiteLoginKind: 'Login kind',
    shareSitePh: 'e.g. my-site (letters, digits, - and _)',
    shareSiteNeeds: 'detected requirements',
    shareSiteSave: 'Add',
    shareSiteCancel: 'Cancel',
    shareSiteRemove: 'Remove site',
    shareSiteLink: 'compose page',
    shareManual: 'Manual posting',
    shareManualHint: 'This site cannot be posted to automatically: the body and the attachment are prepared, and you post them yourself via copy or the site compose page.',
    shareManualCopy: 'Copy body',
    shareManualDownload: 'Download file',
    shareManualOpen: 'Open compose page',
    shareManualFull: 'The body is over this site limit, so the button carries no text (use copy or download)',
    shareTextLimit: 'this site accepts {n} characters',
    ...
    loginNoSession: 'Cookies found but no session cookie (probably not signed in)',
    ...
    cookieCountWithSession: '{n} cookies (incl. a session cookie)',
    ...
    proxyHint:
      'Node fetch ignores the system proxy by default. Enable this if your network requires a proxy — it applies to both scraping and browser rendering. Note: some sites are blocked *because* of a proxy, so sources and watch targets can override this with "direct".',
    ...
    obsRotationHint:
      'Their own sites means entries whose logs stay on their servers (official sites). Platform sources such as Reddit / Fandom do not use Tor: the group cannot see those logs, and measurements show Tor is about 8x slower there and some endpoints rate-limit it. Exit rotation uses Tor SOCKS username isolation — different usernames really do land on different exit IPs.',
    ...
    customSourcesHint:
      'For sites the built-in catalog misses: give it an RSS feed, a MediaWiki API, or a page that needs browser rendering.',
    ...
    watchHint:
      'Not just a site-wide recent-changes firehose: pin one wiki page, one arbitrary URL, or one page. The first check only builds a baseline; every later check reports what actually changed.',
    ...
    intelHint:
      'Items collected by the latest run. Keyword hits are flagged; [emotes] inside a post are marked separately and pictures are shown inline.',
    ...
    llmVisionHint: 'This is a vision model - once configured, the analysis layer can actually look at pictures in a post.',
