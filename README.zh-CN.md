...
- **只读 cookie 探针是通用的**：它只读复制一份浏览器自己的 cookie 库，读取**调用方指定的主机**，只回报 cookie 的**名字**、从不回报值。走的只是「属于那两个站点」的按站用法；机制本身、以及拥有这项设置的那个页面都留着 —— 不过它现在读的是 Firefox（见下），所以原先那些 Chromium 解密说明已经随代码一起删掉了。
...
**只读复制一份浏览器的 cookie 库来读取**，浏览器开着也没关系，不会锁定、不会改动它。
...
- 实测：Firefox 把 cookie 存在 `cookies.sqlite`（表 `moz_cookies`），值是**明文**。没有密钥要找，
  所以也没有会失败的解密步骤 —— 要么读到，要么那个 profile 里就没有该域名的 cookie。
  `host` 就是 Chromium 的 `host_key`，带前导点的约定也一样，所以同名 cookie 里域名 cookie 会压过主机 cookie。
- 一个**不是** Firefox profile 的目录会作为它自己的状态返回（`no-firefox-profile`），而不是「没登录」——
  Chromium 的 profile 不是这个读取器能打开的库，直说比回一个空结果有用。
- 拿到的登录态只用于调用对应站点的接口；**cookie 值不会进日志、不会进报告、不会进 `feeds/`**，
  复制出来的临时库用完即删。接口只回报「读到了哪些 cookie 的名字」，从不回传值。

## 浏览器内核是 Firefox

随包内核是 **Playwright 的 Firefox**（155.0，构建 `firefox-1543`）；Chromium 已经从代码、启动参数、写死的
User-Agent 和打包产物里全部移除。实际意味着这些：

- **只有 Playwright 的 Firefox 构建能被驱动。** Playwright 说的是 Juggler 协议，本机装的 `firefox.exe`
  不实现它，所以原装 Firefox 会在启动前就被拒绝并给出理由，而不是三秒后报「Failed to launch the browser
  process」。消息里直接给了修法：`npx playwright install firefox`。因此 `system` / `custom` 两种模式只提供
  Playwright 内核，旧的那句「指向你已经装好的 Chrome」对这个内核不再成立。
- **本工具启动的每个浏览器都用这个内核** —— `fetch: browser` 的渲染、截图缩略图、UI 遍历都一样。出口也是
  共用的（`server/src/net.js` 的 `resolveBrowserEgress`），和其余抓取走同一道门：Tor 的 SOCKS 端口会在浏览器
  启动**之前**先探测，所以「Tor 没在跑」是一条明确的理由，而且根本不会启动浏览器。截图也搬到了这个出口上 ——
  它以前在 Tor 模式下走的是**直连**，也就是一个隐私开关却在悄悄用本机地址去拍照。
- **一条实测出来的内核限制，如实写出来**：Playwright 的 Firefox 无法向 SOCKS5 认证（它只提供 no-auth 方式，
  并且忽略 Firefox 自己的 `socks_username` 配置）。所以浏览器走 Tor 时用的是**默认线路**；按目标隔离的
  `IsolateSOCKSAuth` 出口轮换，对所有非浏览器抓取照旧生效。
- **实测产物大小**：`--with-browsers` 的内核产物从 705.6 MB（Chromium）降到 **345.3 MB**（Firefox）；
  带浏览器的 zip 是 169.5 MB，不带浏览器的 zip 不变，仍是 40.4 MB。
- **profile 发现读的是 Firefox，不是固定目录结构。** Firefox 的 profile 由安装根目录里的 `profiles.ini`
  指名，而不是躺在 `Default` / `Profile 1` 这类子目录里；而且 profile 属于**这台机器，而不是某一个可执行文件**
  —— 所以随包内核也能复用你现有的 Firefox 登录态，默认 profile 在任何模式下都会被给出。它只是**被给出**
  （一键「用这个」），绝不会自动填进空设置。这一轮顺带修掉三个既有缺陷：文档承诺的默认值在运行时应用里
  一直返回空、`present` 恒为 false、以及真实的 Firefox **根目录**（旧布局会留下零字节的
  `cookies.sqlite` / `places.sqlite`）会被当成 profile，导致里面的 profile 从来没被列出来。
